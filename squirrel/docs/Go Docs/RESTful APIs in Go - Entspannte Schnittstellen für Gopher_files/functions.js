$(document).ready(function() {
	...
	$('.navbar-toggle').on('click', function(e) {
		if (insertedSidebar === false) {
			var navigation = $('#menu-menu').clone();
...
// FIX POSITION OF NAVIGATION WITH DFP BANNER

$(function(){ 
  if ($(window).width() > 991) {
	setTimeout(function() {
	  var theHeightOfNavi = $( 'header .topnav' ).height();
	  var theHeightOfDfpBlock = $( '.topbanner' ).height();
	  var theHeightOfSubnavi   = $( 'header .subnavigation ' ).height();
	  var targetPoint = theHeightOfNavi + theHeightOfDfpBlock + theHeightOfSubnavi;
	  
	  // console.log(theHeightOfNavi);
	  // console.log(theHeightOfDfpBlock);
	  // console.log(targetPoint);
	  
	  $(window).scroll(function(){ 
		    var windowTop = $(window).scrollTop(); 
			if(windowTop > targetPoint){
				$('header .topnav').css({ position: 'fixed', width: '100%' , top: '0px' , zIndex: '1001'});
				$('header .subnavigation ').css({ position: 'fixed', width: '100%' , top: theHeightOfNavi + 'px' , zIndex: '1001'});
			}else{
				$('header .topnav').css({ position: 'relative', width: '100%' , top: 'auto' , zIndex: '0' });
				$('header .subnavigation ').css({ position: 'relative', width: '100%' , top: 'auto' , zIndex: '0'});
			}
	  });
	}, 2000);
  }

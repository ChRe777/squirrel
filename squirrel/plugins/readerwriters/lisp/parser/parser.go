...
func sexpr(level *int) *types.Cell {
	...
	atom := func(level *int) *types.Cell {
		...
		symbol_ := func(level *int) *types.Cell {
			incLevel(level)
			debug("symbol_", level)
			return generator.Atom(scanner.IdStr(), types.SYMBOL)
		}

		string_ := func(level *int) *types.Cell {
			incLevel(level)
			debug("string_", level)
			return generator.Atom(scanner.IdStr(), types.STRING)
		}

		number_ := func(level *int) *types.Cell {
			incLevel(level)
			debug("number_", level)

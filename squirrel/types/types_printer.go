...
func sprintAtom(c *Cell) []byte {
	...
	if c.IsStr() {
		s, _ := c.Val.(string)				// TODO: RETHINK
		ss := fmt.Sprintf("\"%s\"", s)
		buffer.WriteString(ss)
		return buffer.Bytes()
	}

	s := fmt.Sprintf("%v", c.Val)			// TODO: RETHINK
	buffer.WriteString(s)
	
	if c.HasTag() {
		t := fmt.Sprintf("#%v", c.Tag)		// TODO: RETHINK
...
func sprintCons(c *Cell) []byte {

	/*
	var buffer bytes.Buffer

	// TODO: Rethink				"(func#mac (x y) (cons x y))" OR "(func#func (x y) (cons x y))"
...
// 		(cons (1) b) 		-> "((1) . b)"
func sprintDottedPair(c *Cell) []byte {
